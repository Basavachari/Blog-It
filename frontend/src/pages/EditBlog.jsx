...
const EditBlog = () => {
    ...
    const handleEditBlog = async () => {
        setLoading(true)
        if (image) {
            await axios.post(`https://api.cloudinary.com/v1_1/djalm8jjs/image/upload`, formData)
                .then(response => {
            ...
            <div className={`${loading ? 'blur-md' : ''} flex justify-center mt-20 gap-x-44`}>
                ...
                <div className='border-2 border-[#f5f5fa] bg-[#f5f5fa] cursor-pointer rounded-3xl flex justify-center w-2/5'>
                    <label htmlFor="fileInput">
                        <img alt='editimage' src={image ? URL.createObjectURL(image) : blog.imageurl ? `https://res.cloudinary.com/djalm8jjs/image/upload/v${blog.imageurl}.png` : DEFAULT_BLOG_IMAGE} className='h-96 w-full rounded-3xl cursor-pointer' />
                        <input
                            type="file"
                            id="fileInput"
                            className='hidden'
